...
class LoadData(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        x_list = sample['x_list']
        theta = sample['theta']

        # Construct Chain-of-Thought (CoT)
        cot_steps = []
        for i, (text_input, x) in enumerate(zip(text_inputs, x_list)):
            if i < len(text_inputs) - 1:
                cot_steps.append(
                    f"Step {i+1}: The input is \"{text_input}\", which describes a {x} {theta}. "
                    f"The corresponding image shows a {x} {theta}."
                )
            else:
                cot_steps.append(
                    f"Step {i+1}: The input is \"{text_input}\", which describes the target. "
                    f"Based on the pattern, the target is a {x} {theta}."
                )
        
        cot_output = "\n".join(cot_steps) + f"\nAnswer: {x_list[-1]} {theta}"
        instruction = "Use the following steps to predict the target. " + cot_output
        # print(instruction)
