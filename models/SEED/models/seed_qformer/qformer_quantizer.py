...
class Blip2QformerQuantizer(Blip2Base):
    ...
    def get_codebook_entry(self, indices):
        ...
        # print("Step 1: Received input indices")
        # print("Indices shape:", indices.shape)
        # print("Indices", indices)
        quant_embedding = self.quantize.get_codebook_entry(indices)
        # print("\nStep 2: Quant embedding generated.")
        # print("Quant embedding shape:", quant_embedding.shape)
        # print("Quant embedding:", quant_embedding)
        ...
        # print("\nStep 3: Query output up generated.")
        # print("Query output up shape:", query_output_up.shape)
        # print("Query output up:", query_output_up)

        # print("\nStep 4.1: Original position embedding.")
        # print("Original pos_embed_image shape:", self.pos_embed_image.shape)
        # print("Original pos_embed_image:", self.pos_embed_image)

        pos_embed_image = self.pos_embed_image.repeat(query_output_up.shape[0], 1, 1)
        # print('+++++++++++++++++++++++++++')
        # print("\nStep 4.2: Position embedding after repeat.")
        # print("Repeated pos_embed_image shape:", pos_embed_image.shape)
        # print("Repeated pos_embed_image:", pos_embed_image)
        if query_output_up.shape[1] != pos_embed_image.shape[1]:
            print("\n[Warning] Detected shape mismatch!")
            print(f"query_output_up shape: {query_output_up.shape}")
            print(f"pos_embed_image shape: {pos_embed_image.shape}")
            # 如果 query_output_up 更长，裁剪它以匹配 pos_embed_image
            if query_output_up.shape[1] > pos_embed_image.shape[1]:
                query_output_up = query_output_up[:, :pos_embed_image.shape[1], :]
                print(f"query_output_up truncated to: {query_output_up.shape}")
            '''
            # 如果 pos_embed_image 更短，重复它的最后一行以匹配 query_output_up
            elif query_output_up.shape[1] > pos_embed_image.shape[1]:
                pos_embed_image = torch.cat([
                    pos_embed_image,
                    pos_embed_image[:, -1:, :].repeat(1, query_output_up.shape[1] - pos_embed_image.shape[1], 1)
                ], dim=1)
                print(f"pos_embed_image extended to: {pos_embed_image.shape}")
            '''
        ########################################################################
        ...
        if self.use_qformer_image:
            ...
            # if there is more feature length of reverse_output
            if reverse_output.shape[-1] > 1024:
                reverse_output = reverse_output[..., :1024]
            # if there is less feature length of reverse_output
            elif reverse_output.shape[-1] < 1024:
                padding = torch.zeros((reverse_output.shape[0], 1024 - reverse_output.shape[-1]), device=reverse_output.device)
                reverse_output = torch.cat([reverse_output, padding], dim=-1)
                # print("===========================")
                # print(f"reverse_output shape {reverse_output.shape}")
                # print(f"self.distill_image_proj shape {self.distill_image_proj.shape}")
